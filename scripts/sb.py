    ...
    def __init__(self):
        try:
            match sys.argv[1]:
                ...
                case "cb":
                    c = (
                        "ostree remote refs fedora | grep -E "
                        + str(self.sbversion() + 1)
                        + " | grep -E x86_64/silverblue$"
                    )
                    os.system(c)

                case "upgsb":
                    print(
                        "Syntax:\n\nrpm-ostree rebase fedora:fedora/",
                        self.sbversion() + 1,
                        "/x86_64/silverblue",
                    )
                ...
                case "up":
                    os.system(
                        "rpm-ostree upgrade && flatpak update -y && toolbox run sudo dnf update -y"
                    )

                case "mesa-drm-freeworld":
                    os.system(
                        "rpm-ostree override remove mesa-va-drivers --install=mesa-va-drivers-freeworld --install=mesa-vdpau-drivers-freeworld --install=ffmpeg"
                    )
