...
local function corre(file, opt)
	if file:match "%.avif$" or file:match "%.jpg$" or file:match "%.jpeg$" or file:match "%.png$" or file:match "%.webp$" or file:match "%.jxl$" then
		x(("convert %s"):format(opt))
	end
end

local function resize(size)
	mkdir()
	for file in ls() do corre(file, ([[-resize %s "%s" "resultado/%s"]]):format(size, file, file)) end
end

local function normalize_ppi()
	mkdir()
	for file in ls() do corre(file, ([[-depth 72 "%s" "www/%s"]]):format(file, file)) end
end
